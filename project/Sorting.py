...
def create_itog(Window, files_df):
    """Функция для создания таблицы для заказа
    ...
    try:
        ...
        if len(df_r) > 2:
            ...
            df = pd.concat([df, df_r[1]], ignore_index=True)
        ...
        final_df = final_df.sort_values(by=['Код 1 С', 'Тип и техническая характеристика'],
                                        ascending=[False, True], ).reset_index().drop(['index'], axis=1).reset_index()
        final_df['index'] += 1
...
def create_svod_table(df, df_one_yach, files_df):
    ...
    Parameters:
    df: DataFrame
    df_one_yach: DataFrame
    files_df: DataFrame
    """
    for n in range(len(df_one_yach)):
        df_one = df_one_yach[n].copy(deep=True)
        df_one.rename(columns={'Кол.': f'{files_df[n]["title_cell"]}', 'Кол. общ': f'{files_df[n]["title_cell"]} общ'},
                      inplace=True)
